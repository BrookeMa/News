...
public final class CoreDataArticleStore: ArticleStore {
        ...
        perform { context in
            completion(Result {
                try ManagedCache.find(in: context).map {
                    return CachedArticle(articles: $0.localArticles, timestamp: $0.timestamp)
                }
            })
        }
    }
    ...
    public func insert(_ articles: [LocalArticle], timestamp: Date, completion: @escaping InsertionCompletion) {
        perform { context in
            completion(Result {
                let managedCache = try ManagedCache.newUniqueInstance(in: context)
                managedCache.timestamp = timestamp
                managedCache.articles = ManagedArticle.items(from: articles, in: context)
                try context.save()
            })
        }
    }
